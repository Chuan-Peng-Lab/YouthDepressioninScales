...
png("figure/unmergebootEGA.png", width = 3300, height = 2000, res = 300)
...
# 保存新的数据框为Excel文件
library(writexl)
write_xlsx(new_2_network_data, "data/new_2_network_data.xlsx")
...
# 基于Unique Variable Analysis结果进行EGA分析
merge_ega <- EGA(data = merge_uva$reduced_data,plot.EGA = TRUE, label.size = 2,legend.names = c("Dimension 1", "Dimension 2", "Dimension 3", "Dimension 4"),
...
new_2_network_data_filtered <- new_2_network_data %>%
  select(-Suicidal_ideation, -Interest_loss, -Never_do_what_I_am_told, 
         -Get_into_fights_all_the_time, -Lack_of_friends) 
...
new_2_network_data_filtered_second <- new_2_network_data %>%
  select(-Suicidal_ideation, -Interest_loss, -Never_do_what_I_am_told, 
         -Get_into_fights_all_the_time, -Lack_of_friends,  # 按照新的稳定性结果删除这些变量
...
```{r}
# 更改名字
# 创建更新后的映射字典，将原始列名映射为新的简洁列名
column_mapping <- c(
  "Sad" = "M1", 
  "Anhedonia" = "M2", 
  "Psychological_worry" = "M3", 
  "Cry" = "M4", 
  "Psychic_anxiety" = "M5", 
  "Fatigue" = "M6", 
  "Appetite_change" = "M7", 
  "Somatic_worry" = "M8", 
  "Feeling_of_loneliness" = "M9", 
  "Poor_sleep" = "M10", 
  "Gastrointestinal" = "M11", 
  "Low_mood" = "M12", 
  "Blue" = "M13", 
  "Feeling_of_hopelessness" = "C1",
  "Psychological_inferiority" = "C2",
  "Self_blame" = "C3",
  "Felt_people_disliked_me" = "C4",
  "Running_away_from_home" = "C5",
  "Interest_pleasure_loss" = "C6",
  "Feeling_of_worthlessness" = "C7",
  "Negative_body_perception" = "C8",
  "Like_talking_with_family" = "C9",
  "Reduced_socialization" = "C10",
  "Inferiority_self_confidence" = "C11",
  "Bad_all_the_time" = "C12",
  "Learning_difficulties" = "F1",  # F组按顺序1-6排列
  "Loss_of_energy" = "F2",
  "Concentration" = "F3",
  "Retardation_and_agitation" = "F4",
  "Never_have_fun_at_school" = "F5",
  "Indecisiveness" = "F6"
)

# 创建一个新的数据框并为其赋予原始数据
new_2_network_data_filtered_renamed <- new_2_network_data_filtered_second

# 获取需要更改列名的列（这些列在映射字典中有定义）
cols_to_rename <- intersect(colnames(new_2_network_data_filtered_renamed), names(column_mapping))

# 只对需要修改列的列名进行修改
colnames(new_2_network_data_filtered_renamed)[colnames(new_2_network_data_filtered_renamed) %in% cols_to_rename] <- column_mapping[cols_to_rename]

# 查看新的数据框的列名
colnames(new_2_network_data_filtered_renamed)

# 执行 Unique Variable Analysis（唯一变量分析）
merge_uva_filtered_renamed <- UVA(
  data = new_2_network_data_filtered_renamed,  # 使用重命名后的数据
  key = colnames(new_2_network_data_filtered_renamed)  # 使用变量名作为key
)

# 查看结果

merge_uva_filtered_renamed 
merge_uva_filtered_renamed$keep_remove  # 查看哪些变量需要保留（keep）或删除（remove）

# 设置图像保存路径并绘制更美观的 EGA 网络图
png("figure/EGA_network_plot_filtered_renamed.png", width = 3300, height = 2000, res = 300)

# 基于 Unique Variable Analysis 结果进行 EGA 分析
merge_ega_filtered_renamed <- EGA(
  data = merge_uva_filtered_renamed$reduced_data,  # 使用排除掉不需要列后的数据
  plot.EGA = TRUE,
  label.size = 3,  # 增加标签的大小以便阅读
  legend.names = c("MOOD", "COGNITION", "FUNCTION"),
  node.size = 18,  # 增加节点大小让图形更清晰
  edge.width = 2,  # 增加边的宽度使关系更加清晰
...
# 查看 EGA 分析结果
summary(merge_ega_filtered_renamed)
