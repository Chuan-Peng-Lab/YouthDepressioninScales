...
# 读取矩阵
rawdata <- read_excel(here::here("DATA", "Depression_Matrix.xlsx"))
...
order_data <- rawdata %>%
  select(-face, -color) %>%
  mutate(count = rowSums(across(where(is.numeric)), na.rm = TRUE)) %>%
  arrange(desc(count)) %>%
  select(-count)

d <- order_data %>%
  select(-S_name) %>%
  mutate(S = paste(1:nrow(order_data))) %>%
  reshape2::melt(id.vars = "S", variable.name = "Scales", value.name = "Type") %>%
  filter(Type >= 1) %>%
  mutate(
    Type = ifelse(Type == 1,
...
symp <- order_data %>%
  arrange(desc(row_number())) %>%  # 倒序症状
  mutate(S = nrow(.) : 1) %>%
  mutate(S_name = factor(S_name, levels = .$S_name)) %>%
  left_join(rawdata, by = "S_name") %>%
  select(S, S_name, face, color)
...
CA_plot <- full_join(num, d, by = "Scales") %>%
  mutate(
    Scales2 = order,
    Scales = factor(Scales, levels = num$Scales),
    S = factor(S, levels = symp$S, labels = symp$S_name)
  ) %>%
  select(S, Scales, Scales2, Type)

scale <- read_excel(here::here("DATA", "Depression_Matrix_extra.xlsx"))
...
plot[[1]] <- CA_plot %>%
  ...
  # 背景网格
  geom_hline(yintercept = 1:n_scales, color = "grey80", linewidth = 0.2) +
  geom_vline(xintercept = 1:n_symptoms, color = "grey80", linewidth = 0.2) +
  ...
  # =========== ⬇⬇⬇ 镜像翻转的关键行（仅这一行）⬇⬇⬇ ===========
  scale_x_discrete(
    limits = rev(levels(CA_plot$S)),   # ★ X 轴完全左右翻转（镜像）
    drop = FALSE
  ) +
  # ===========================================================
  ...
  theme(
    axis.text.x = element_text(
      ...
      color = case_when(
        rev(symp$color) == 0 ~ "black",
        rev(symp$color) == 1 ~ "#5a9ad7",
        rev(symp$color) == 2 ~ "#ff0000"
      )
    ),
    axis.text.y = element_text(
      color = case_when(
...
ggsave(
  filename = here::here("FIGURE", "1_Content_Analysis_LEFT_RIGHT_FLIPPED.png"),
  width = 32,
  height = 18,
  dpi = 300
)
...
my_breaks <- c(0.0, 0.2, 0.4, 0.6, 0.8)

grDevices::pdf(
  file = here::here("FIGURE", "2_Heatmap.pdf"),
  width = 16,
  height = 12
)

# 先画热图，并保存 corrplot 返回结果
corr_res <- corrplot::corrplot(
  df_ji,
  method = "circle",
  addCoef.col = "black",
  diag = FALSE,
...
# ============================================================
ggsave(
  filename = here::here("FIGURE", "2_Heatmap_Distribution.png"),
  plot = p,
  width = 10,
  height = 7,
  dpi = 300
